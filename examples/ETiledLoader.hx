...
class ETiledLoader extends TiledLoader {
	override private function loadObject(state:BState, o:TiledObject, g:TiledObjectLayer, group:FlxGroup) {
		...
		if (_flipX == null)
			throw "'flipX' property not defined for the '" + o.name + "' layer. Please add the property to the layer.";

		var controller:String = o.properties.get("controller");
		if (controller == null)
			throw "'controller' property not defined for the '" + o.name + "' layer. Please add the property to the layer.";
