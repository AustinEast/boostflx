...
class ArrayUtil {
	...
	public static function make_outlined_2D_array(width:Int, height:Int, outlinewidth:Int = 1, outlineheight:Int = 1, innerInt:Int = 1, outlineInt = 0):Array<Array<Int>> {
		return [
			for (i in 0...height + outlineheight + outlineheight)
				[
					for (j in 0...width + outlinewidth + outlinewidth)
						(j < outlinewidth || j >= width + outlinewidth || i < outlineheight || i >= height + outlineheight) ? outlineInt : innerInt]];
	}
