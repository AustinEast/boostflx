...
class ControlsManager extends FlxBasic
{
	...
	{
		if(!acceptDeactiveControllerInput && !isActiveController(CONTROLLER)) return false;
	...
	public function anyJustPressed (BUTTON:Button):Bool
	{
		...
		{
			if (justPressed(BUTTON, intToController(i))) return true;
		}
		return false;
	}
	...
	public function anyJustReleased (BUTTON:Button):Bool
	{
		for (i in 0...MAX_CONTROLLERS) 
		{
			if (justReleased(BUTTON, intToController(i))) return true;
		}
		return false;
	}
	...
	public function anyPressed (BUTTON:Button):Bool
	{
		for (i in 0...MAX_CONTROLLERS) 
		{
			if (pressed(BUTTON, intToController(i))) return true;
		}
		return false;
	}
	...
	{
		var n = 0;
		for(i in 0...activeControllers.length) { if (activeControllers[i]) n++; }
		return n;
	}
}