...
class HitBoxManager extends FlxTypedGroup<HitBox> {
	...
	public function activate(x:Float, y:Float, width:Int, height:Int, parent:Dynamic, variable:String = "", removeIfTrue:Bool = true) {
		if (getFirstOpen() != null)
			getFirstOpen().activate(x, y, width, height, parent, variable, removeIfTrue);
		else {
...
class HitBox extends FlxObject {
	...
	override public function toString():String {
		return FlxStringUtil.getDebugString([LabelValuePair.weak("value", value)]);
	}
	...
	private function set_value(newValue:Bool):Bool {
		value = newValue;
		if (value == removeIfTrue)
			kill();
		else if (reviveIfTrue && value != removeIfTrue)
			revive();
		return newValue;
	}
