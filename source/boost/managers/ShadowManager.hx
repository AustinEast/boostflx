...
class ShadowManager extends FlxTypedGroup<Shadow> {
	...
	public function activate(target:FlxSprite, img:String, img_width:Int, img_height:Int, frame:Int = 0):Shadow {
		if (getFirstOpen() != null)
			return getFirstOpen().activate(target, img, img_width, img_height, frame);
		else
			return add(new Shadow(target.x, target.y).activate(target, img, img_width, img_height, frame));
	}
	...
	override public function update(elapsed:Float):Void {
		if (target != null && target.exists) {
			...
			if (matchTargetAngle)
				angle = target.angle;
			if (matchTargetFrame)
				animation.frameIndex = target.animation.frameIndex;
